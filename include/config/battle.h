...
#define B_FLAG_NO_BAG_USE           FLAG_SYS_NO_BAG_USE              // If this flag is set, the ability to use the bag in battle is disabled.
#define B_FLAG_NO_CATCHING          FLAG_SYS_NO_CATCHING     // If this flag is set, the ability to catch wild Pokémon is disabled.

// Var Settings
// To use the following features in scripting, replace the 0s with the var ID you're assigning it to.
// Eg: Replace with VAR_UNUSED_0x40F7 so you can use VAR_TERRAIN for that feature.
#define VAR_TERRAIN                 VAR_PERMA_TERRAIN     // If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active
...
#define B_OBEDIENCE_MECHANICS       GEN_7 // In PLA+ (here Gen8+), obedience restrictions also apply to non-outsider Pokémon, albeit based on their level met rather than actual level
                                               // PSF TODO switch this back to GEN_LATEST when for public release
