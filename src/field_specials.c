...
//static EWRAM_DATA u16 sScrollableMultichoice_ScrollOffset = 0; //https://github.com/pret/pokeemerald/compare/master...SBird1337:pokeemerald:feature/dynmulti
...
bool32 ShouldDoScottFortreeCall(void)
{
    ...
    if (VarGet(VAR_STORYLINE_STATE) == STORY_POST_YOU_REALIZE_WERE_EVIL && FlagGet(FLAG_ARCHER_CALL_ALCATRAZ))
    //if (FlagGet(FLAG_SCOTT_CALL_FORTREE_GYM))
    {
        ...
        case MAP_TYPE_ROUTE:
        case MAP_TYPE_OCEAN_ROUTE:
            if (++(*GetVarPointer(VAR_ARCHER_CALL_ALCATRAZ_STEP_COUNTER)) < 10)
                return FALSE;
            break;
        default:
            return FALSE;
...
static void Task_ShowScrollableMultichoice(u8 taskId)
{
    ...
    LockPlayerFieldControls();
    //sScrollableMultichoice_ScrollOffset = 0; //https://github.com/pret/pokeemerald/compare/master...SBird1337:pokeemerald:feature/dynmulti
...
static void ScrollableMultichoice_MoveCursor(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    ...
    if (taskId != TASK_NONE)
    {
        ...
        //sScrollableMultichoice_ScrollOffset = selection; //https://github.com/pret/pokeemerald/compare/master...SBird1337:pokeemerald:feature/dynmulti
...
static void ScrollableMultichoice_UpdateScrollArrows(u8 taskId)
{
    ...
    if (task->tMaxItemsOnScreen != task->tNumItems)
    {
        ...
        //task->tScrollArrowId = AddScrollIndicatorArrowPair(&template, &sScrollableMultichoice_ScrollOffset); //https://github.com/pret/pokeemerald/compare/master...SBird1337:pokeemerald:feature/dynmulti
...
bool8 CheckMegaEvolutionPartyMember(void)
{
    ...
    for (PartySlot = PARTY_SIZE; PartySlot > -1; PartySlot--)
    {
        int IsEvolution = 0;
        species = GetMonData(&gPlayerParty[PartySlot], MON_DATA_SPECIES, NULL);
        GetMegaEvolutionPartyMember(species,PartySlot,IsEvolution);
    }
}

bool32 GetMegaEvolutionPartyMember(u16 species, int PartySlot, int IsEvolution)
{
    int i;
    for (i = 0; i < EVOS_PER_MON; i++)
    {
        ...
        {
            if (gEvolutionTable[species][i].method == SPECIES_FLAG_MEGA_EVOLUTION)
            {
